...
# -*- coding: utf-8 -*-
"""
Created on Sun May  8 12:40:44 2022
...
print('Knn Score variance : ', knn_cv_variance) #84%
...
print('gb Score variance : ', gb_cv_variance) #92% accuracy
...
# Since the Model is chosen now, we do hyperparameter tuning using GridSearch method 
# that is looped. We use Random Forest for this.
...
n_estimators_List = [100,150,200] # range(25,100,25)
max_features_List = [5, 7, 9] # range(5,11,2)
min_samples_leaf_List = [5, 10, 25, 50,100]
...
# You choose the n_estimators, max_features, min_samples_leaf values from the first
# ranked model of Model_Validation_Df4 

# Based on the selected hyperparamters, you should build a final model on the COMPLETE training data (trainX, trainY)
gb = RandomForestClassifier(random_state = 4848, n_estimators = 100, 
                               max_features = 5, min_samples_leaf = 5).fit(X_train, y_train)

# You check accuracy %
gb_accuracy = gb.score(X_test, y_test)*100  
print(gb_accuracy) #which is 89% accuracy

# Predicting the final values
Test_Pred = RF_Final.predict(predicdf)
...
OutputDf.to_csv("submission_code.csv",index=False)
